...
package net.pixomania.crawler.W3C.parser.rules.principalContributors;
...
public class PrincipalContributorsRule2 implements Rule<ArrayList<Person>> {
	...
	public ArrayList<Person> run(String url, Document doc) {
		...
		Elements editors = doc.select("h4:contains(Principal Contributor), h4:contains(Principal Contributor) ~ blockquote");
		if (editors.size() == 0) return null;

		boolean skip = false;
		for (Element editor : editors) {
			Element prev = editor.previousElementSibling();
			if (!prev.tagName().equals("blockquote")) {
				if (!prev.text().trim().toLowerCase().startsWith("principal contributor")) {
					skip = true;
				}
			}
			...
			if (skip) {
				...
				if (next != null) {
					if (next.text().trim().toLowerCase().startsWith("principal contributor")) {
						skip = false;
						continue;
					}
				}
			...
			if (splitted.length < 2) {
				if (editor.text().toLowerCase().startsWith("english, omg idl")) continue;
				...
				for (String split : splitted) {
					if (!split.isEmpty()) {
						if (split.toLowerCase().startsWith("english, omg idl")) continue;
